...
def start_regular_server():
    ...
    server.listen(50)
    console.print(f"[green]🌽 CornProxy (regular) listening on 127.0.0.1:{listen_port}[/green]")
    while running:
        try:
...
def tui_loop():
    ...
    with Live(layout, refresh_per_second=2, screen=True) as live:
        while running:
            ...
            stat_table.add_row("MITM", "ON" if mitm_mode else "OFF")
...
def main():
    ...
    console.print(Panel.fit("[bold green]CornProxy v4.0 - MITM + Anti-DPI + Pool[/bold green]", border_style="green"))
