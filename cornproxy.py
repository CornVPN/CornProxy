#!/usr/bin/env python3
# CornProxy v3.3 - прокси-логгер с пулом, DPI, пингованием и TUI
...
def update_proxy_pool(fetch_from_web=True):
    ...
    good.sort(key=lambda x: x[5])  # sort by ping (lower is better)
...
def get_next_proxy():
    ...
    with proxy_pool_lock:
        ...
        proxy_pool_index += 1
        p = proxy_pool[idx]
        current_proxy = (p[0], p[1], p[2], p[3], p[4])  # without ping
...
def main():
    global use_direct, listen_port, proxy_pool, current_proxy
    console.clear()
    ascii_art = pyfiglet.figlet_format("CornProxy", font="slant")
    console.print(ascii_art, style="bold cyan")
    console.print(Panel.fit("[bold green]Welcome! Proxy logger with ping and auto pool[/bold green]", border_style="green"))
    ...
    if mode_choice == "3":
        ...
        proxy_pool = [("http", proxy_host, proxy_port, user, pwd, 0.0)]  # ping placeholder
