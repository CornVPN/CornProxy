...
listen_port = 8888
use_direct = False
proxy_pool = []               # (type, host, port, user, pass)
proxy_pool_lock = threading.Lock()
proxy_pool_index = 0
dpi_mode = "off"              # off, fragment, random_case, noise
...
def connect_http_proxy(proxy_host, proxy_port, target_host, target_port, user=None, password=None):
    """Подключается к HTTP-прокси и отправляет CONNECT. Возвращает сокет туннеля."""
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(30)
...
def send_http_via_proxy(proxy_host, proxy_port, request, user=None, password=None):
    """Отправляет HTTP-запрос через HTTP-прокси (абсолютный URL уже в запросе)."""
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect((proxy_host, proxy_port))
    if user and password:
        # Добавляем заголовок Proxy-Authorization, если его нет в запросе
        auth = base64.b64encode(f"{user}:{password}".encode()).decode()
...
def handle_client(client_sock):
    ...
    try:
        ...
        if method.upper() == 'CONNECT':
            ...
            # Переписываем запрос в абсолютную форму для HTTP-прокси
            # То есть "GET http://example.com/path HTTP/1.1"
            # Заменяем относительный путь на полный URL
            if not url.startswith('http'):
                # уже относительный? превращаем в абсолютный
                new_url = f"http://{target_host}:{target_port}{parsed.path or '/'}"
                if parsed.query:
                    new_url += '?' + parsed.query
                request = request.replace(url.encode(), new_url.encode())
            # Для SOCKS или direct оставляем как есть

        if use_direct:
            # Прямое соединение (без внешнего прокси)
            ...
            if proto == "http":
                if method.upper() == 'CONNECT':
                    # Устанавливаем туннель через CONNECT
                    ...
                    # Обычный HTTP-запрос через HTTP-прокси (с абсолютным URL)
                ...
                # Для SOCKS используем библиотеку socks, которая сама устанавливает туннель
                if method.upper() == 'CONNECT':
                    # SOCKS туннель устанавливается сразу при connect
